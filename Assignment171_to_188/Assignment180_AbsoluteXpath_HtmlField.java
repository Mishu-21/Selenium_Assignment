////Write an Absolute xpath and relative xpath for the html file(shared my Manish) for all the fields

package Assignment;

import org.openqa.selenium.By;
import org.openqa.selenium.WebElement;
import org.openqa.selenium.chrome.ChromeDriver;
import org.openqa.selenium.support.ui.Select;

public class Assignment180_AbsoluteXpath_HtmlField {

	public static void main(String[] args) throws InterruptedException {
		// TODO Auto-generated method stub

		ChromeDriver driver=new ChromeDriver();
		driver.get("file:///C:/Users/Brijesh%20Chauhan/Desktop/imp/Study_Himanshi/GroTechMinds/Study%20Material/learningHTML1.html");
		driver.manage().window().maximize();
		Thread.sleep(500);
		
		//userfield
		driver.findElement(By.xpath("(/html/body/input)[1]")).sendKeys("Himanshi21");
		Thread.sleep(500);
		
		//hint field
		driver.findElement(By.xpath("(/html/body/input)[2]")).sendKeys("Mishu*");
		Thread.sleep(500);
		
		//pswd field
				driver.findElement(By.xpath("(/html/body/input)[3]")).sendKeys("mypswd");
				Thread.sleep(500);
				
		//first name field
		driver.findElement(By.xpath("(/html/body/form/input)[1]")).sendKeys("Himanshi");
		Thread.sleep(500);
		
		//last name field
		/*
		 * driver.findElement(By.xpath("(/html/body/form/input)[2]")).sendKeys("Chauhan"
		 * ); Thread.sleep(500); as this field is disable so when we try to send any
		 * text into it It will show
		 * "org.openqa.selenium.ElementNotInteractableException" Exception
		 */
		
		//submit button 
		//driver.findElement(By.xpath("(/html/body/form/input)[3]")).click();
		//when we are  click on it it will show us "Your file cannot be accessed so lets dont click it "
		driver.findElement(By.xpath("(/html/body/form/input)[3]"));
		Thread.sleep(500);
		
		//I have a boy checkbox
		driver.findElement(By.xpath("((/html/body/form)[2]/input)[1]")).click();
		Thread.sleep(500);
		
		//I Have a girl checkbox
		driver.findElement(By.xpath("((/html/body/form)[2]/input)[2]")).click();
		Thread.sleep(500);
		
		//i have a baby check box and dont click on it 
		driver.findElement(By.xpath("((/html/body/form)[2]/input)[3]"));
		Thread.sleep(500);
		
		//submit button 
				//driver.findElement(By.xpath("(/html/body/form/input)[3]")).click();
				//when we are  click on it it will show us "Your file cannot be accessed so lets dont click it "
				driver.findElement(By.xpath("((/html/body/form)[2]/input)[4]"));
				Thread.sleep(500);
				
				//who are u? male or female field select checkbox
				driver.findElement(By.xpath("(/html/body/input)[4]")).click(); //male field select 
				Thread.sleep(500);
				driver.findElement(By.xpath("(/html/body/input)[5]")).click();  //female field select 
				Thread.sleep(500);
				
				//Are you ready to relocate to bangalore? field checkbox
				driver.findElement(By.xpath("(/html/body/input)[6]")).click();  //female field select 
				Thread.sleep(500);
				
				//from the dropdown select religion
				WebElement religion=driver.findElement(By.xpath("/html/body/select"));
				Select s=new Select(religion);
				s.selectByIndex(1);
				
				//signup button field--> as when click so not going anywhere
				driver.findElement(By.xpath("(/html/body/input)[7]")).click(); 
				Thread.sleep(500);
				
				
				//Click to know about us --> it will take us to another website 
				driver.findElement(By.xpath("/html/body/a")).click(); 
				Thread.sleep(1000);
				
				//relaunching 
				driver.get("file:///C:/Users/Administrator/Desktop/imp/Study_Himanshi/GroTechMinds/Study%20Material/learningHTML1.html");
				Thread.sleep(2000);
				driver.close();
	}

}
